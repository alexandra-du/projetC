...
// Importation des Données Google depuis Yahoo finance
    char url1[400];
...
//Données Apple
    ...
    sprintf(url4, "wget --no-check-certificate --load-cookies=cookie.txt \"https://query1.finance.yahoo.com/v7/finance/download/AAPL?period1=1509577200&period2=1512082800&interval=1d&events=history&crumb=%s\" -O APPLE.csv", token4);
    system(url4);
...
//Données Microsoft
    ...
    sprintf(url5, "wget --no-check-certificate --load-cookies=cookie.txt \"https://query1.finance.yahoo.com/v7/finance/download/MSFT?period1=1509577200&period2=1512082800&interval=1d&events=history&crumb=%s\" -O MICROSOFT.csv", token5);
    system(url5);
...
char tabCoursAmazon[60][50]; //on crée un tableau contenant les dates et les cours journaliers à la clôture correspondants pour Amazon
    ...
    char ligne2[500]; //ligne2 lit ligne par ligne le fichier GOOG.csv
    char *token12;
    int i2=0;
   
    fgets(ligne2,500, input2); //ligne2 commence par stocker la première ligne du CSV
    while(fgets(ligne2,500,input2)!=NULL){
      /* get the first token */
        ...
        while( token12 != NULL ) { //tant qu'on ne parvient pas à la fin de la ligne
            ...
            strcpy(tabCoursAmazon[i],token12); //on copie chaque token12 dans le tableau prévu à cet effet
    ...
    char ligne3[500]; //ligne3 lit ligne par ligne le fichier GOOG.csv
    char *token13;
    int i3=0;
   
    fgets(ligne3,500, input3); //ligne3 commence par stocker la première ligne du CSV
    while(fgets(ligne3,500,input3)!=NULL){
      /* get the first token */
        ...
        while( token13 != NULL ) { //tant qu'on ne parvient pas à la fin de la ligne
            ...
            strcpy(tabCoursFacebook[i],token13); //on copie chaque token3 dans le tableau prévu à cet effet
...
char tabCoursApple[60][50]; //on crée un tableau contenant les dates et les cours journaliers à la clôture correspondants pour Apple
    ...
    char ligne4[500]; //ligne4 lit ligne par ligne le fichier APPLE.csv
    char *token14;
    int i4=0;
   
    fgets(ligne4,500, input4); //ligne4 commence par stocker la première ligne du CSV
    while(fgets(ligne4,500,input4)!=NULL){
      /* get the first token */
        ...
        while( token14 != NULL ) { //tant qu'on ne parvient pas à la fin de la ligne
            ...
            strcpy(tabCoursApple[i],token14); //on copie chaque token4 dans le tableau prévu à cet effet
    ...
    char ligne5[500]; //ligne5 lit ligne par ligne le fichier MICROSOFT.csv
    char *token15;
    int i5=0;
   
    fgets(ligne5,500, input5); //ligne5 commence par stocker la première ligne du CSV
    while(fgets(ligne5,500,input5)!=NULL){
      /* get the first token */
        ...
        while( token15 != NULL ) { //tant qu'on ne parvient pas à la fin de la ligne
            ...
            strcpy(tabCoursMicrosoft[i],token15); //on copie chaque token5 dans le tableau prévu à cet effet
